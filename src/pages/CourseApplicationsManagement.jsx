...
const CourseApplicationsManagement = () => {
  ...
  const handleApprove = async (applicationId) => {
    try {
      ...
      await api.put(`/course-applications/${applicationId}/approve`);
  ...
  const handleReject = async (applicationId, reason) => {
    try {
      setProcessingId(applicationId);
      await api.put(`/course-applications/${applicationId}/reject`, {
  ...
  const filteredApplications = applications.filter(app => {
    const matchesSearch = 
      app.course?.code?.toLowerCase().includes(searchTerm.toLowerCase()) ||
        ...
        >
          <div>
            ...
            <p className="text-slate-600 dark:text-slate-400">
              Öğretmenlerin derslere yaptığı başvuruları yönetin
            </p>
          </div>
        </motion.div>
                ...
                <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                  <div className="flex-1 space-y-2">
                    ...
                    <p className="text-sm text-slate-600 dark:text-slate-400">
                      {app.course?.name || 'Ders Adı'}
                    </p>
                    <div className="flex flex-wrap gap-4 text-sm text-slate-600 dark:text-slate-400">
                      <div className="flex items-center gap-2">
                        <BookOpen className="w-4 h-4" />
                        <span>Kredi: {app.course?.credits} | ECTS: {app.course?.ects}</span>
                      </div>
                      <div className="flex items-center gap-2">
                      ...
                      {app.course?.department && (
                        <div className="flex items-center gap-2">
                          <Filter className="w-4 h-4" />
                          <span>{app.course.department.name}</span>
                        </div>
                      )}
                    </div>
                    {app.rejectionReason && (
...
export default CourseApplicationsManagement;
